...
use crate::domain::{Init, Mode, Prompt};
use crate::printer;

pub fn prompt(cmd: &Prompt, backend: &dyn Backend, ctx: &Context) {
...
fn prompt_copy(resp: &str, ctx: &Context) {
    ...
    match read_line() {
        Ok(l) if l.trim().is_empty() => match copy_to_clipboard(resp) {
            Ok(_) => println!("Copied"),
            Err(err) => {
...
pub fn init(_cmd: &Init, fp: &PathBuf, ctx: &Context) {
    ...
    if let Some(parent) = fp.parent()
        && !parent.exists()
    {
        ctx.vprint(format_args!("creating config directory {:?}", parent));
    ...
    if let Err(e) = fs::write(fp, crate::r#static::example_config::CONFIG_STRING) {
        eprintln!("nelson: failed to write config file {:?}: {}", fp, e);
    }
}
